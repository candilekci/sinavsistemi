...
namespace SinavSistemi
{
    public unsafe partial class frmTest : Form
    {
        ...
        SqlConnection baglanti = new SqlConnection("Data Source=DESKTOP-SP1FMDH\\MSSQLSERVER01;Initial Catalog=virtualClassDB;Integrated Security=True");
        ...
        public void ilkTest()
        {
            ...
            for (int i = 0; i < secilenKonular.Length; i++)
                {
                    SqlCommand sorular = new SqlCommand("select top(7) * from sorular where konuid =" + secilenKonular[i] + "order by NEWID()", baglanti);
                    baglanti.Open();
                    SqlDataReader sonuc = sorular.ExecuteReader();
                    while (sonuc.Read())
                    {
        ...
        public void Sinav(ref int deger)
        {
            ...
            {
                SqlCommand komut = new SqlCommand(("select top(10) * from sonuclar where userid="+girenID + "order by id DESC"), baglanti);
                baglanti.Open();
                SqlDataReader sdr = komut.ExecuteReader();
                while (sdr.Read())
                {
                ...
                baglanti.Close();
                this.Hide();
                yanlisSayisi = (10 - dogruSayisi.Count);
